...
Para añadir una arista al nodo '1' en el grafo $G$ de forma equiprobable, recorremos los nodos '3' hasta '99' (los nodos '2' y '100' ya son vecinos de '1') y creamos la arista para unir estos nodos sobre el grafo original $G$. A continuación, obtenemos la distancia media nueva y el diametro nuevo y guardamos estos valores, para cada iteración, en un conjunto ($Avg_D$ guarda las distancias media y $Diam$ guarda los diametros).

Importante ver que generamos 97 nuevos grafos aro con una arista extra añadida entre dos nodos, no generamos 1 grafo aro con 97 aristas más.

Finalmente, realizamos la media de los dos conjuntos para obtener la distancia media y el diametro.
...
La diferencia con el método anterior es que con Monte Carlo nuestros dos conjuntos tendrán 10.000 valores (y no 97 como antes). Además, no se realiza un recorrido de todos los nodos, sino que, de forma aleatoria, elegimos uno de los 97 nodos para crear una arista entre este nodo y el nodo '1'.

Importante ver que, mediante la simple aleatoriedad, podemos obtener valores repetidos porque podemos generamos el mismo grafo varias veces, mientras que en el caso anterior nunca generamos dos veces el mismo grafo.

En promedio, el resultado de utilizar el primer método o utilizar el método de Monte Carlo es el mismo.
```{r}
Avg_D = c()
Diam = c()
for(i in 1:10000){
...
# LINK --> http://pages.stat.wisc.edu/~karlrohe/netsci/MeasuringTrianglesInGraphs.pdf
# link 2 --> http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.70.9248&rep=rep1&type=pdf
...
for (i in 5:2000) {
  ...
  full_g = make_full_graph(c)
  line_g = graph(sort(cbind(seq(c, c+l-1), seq(c+1, c+l))), directed = FALSE)
  join_g = graph.union(full_g, line_g)
  
  trans[i] = transitivity(join_g)
  clus[i] = transitivity(join_g, type="average")
}
...
```
```{r}
g <- graph_from_literal(1---2, 2---3, 3---4, 4---5, 5---2, 2---4)
plot(g)
transitivity(g)
