...
$$
\begin{array}{rcl}
\log\left( \frac{P(A_{ij}=1|G_{ij}^c)}{P(A_{ij}=0|G_{ij}^c)}\right) &=&  -3.39393  \cdot \Delta(\mbox{nombre de relacions homosexuals})+ 2.23508  \cdot \Delta(\mbox{nombre de nodes de grau 1})\\
&&  -0.14517  \cdot \Delta(\mbox{suma diferències d'edat entre nodes adjacents})\\
&&  + 0.62781  \cdot \Delta(\mbox{nombre de relacions entre mateixa raça})\\
&&  -0.30575  \cdot \Delta(\mbox{suma diferències en el nombre de la temporada}),\\
\end{array}
$$
on recordem que,
...
**(d)** _What approach do you consider more reasonable to answer questions **(a)** and **(b)** for this specific network, the ERGM one or the SBM one? Justify your answer._
</ul>
