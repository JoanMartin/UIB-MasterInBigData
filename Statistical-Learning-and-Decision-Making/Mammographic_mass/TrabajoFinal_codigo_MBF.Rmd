...
**Nota:** No aparece texto explicativo sobre en análisi datos, sinó algunos comentarios para acompanyar el código que se aplica a cada caso. Así, el documento *TrabajoFinal_memoria_MBF.pdf* es el documento principal, con las explicaciones correspondientes.
...
Excepto la variable **Age**, las variables son cualitativas.<br>
...
Además tenemos 5 ceros pero, según la documentación, BI-RADS toma valores entre 1 y 5.<br>
Aún así, el valor cero si que existe en la classificación de BI-RADS.

Para evitar tener observaciones erroneas, los quitamos.<br>
...
Salvo la edad, no es conveniente cambiar los NaN por algun otro valor.<br>
...
Tal como sigue, R reconoce las variables como factores, pero para poder aplicar la mayoría de métodos, necesitamos que sean de tipo numérico.
```{r}
...
Tomamos 2/3 de los datos aleatoriamente y los guardamos como train.<br>
...
Se mantiene. <br> 
Ya tenemos los conjuntos de entrenamiento y validación.
...
En cada plot indicamos por colores si cada observación pertenece a la classe benigno o maligno. Además, sólo realizamos el plot sobre train (test sólo es para simular el error fuera de la muestra).
...
Quitamos las columnas que serán la categoria referencia.
...
Para aplicar K-nearest neighbors, necesitamos especificar el número de vecinos (K). Así, usamos validación cruzada en train para elegir la mejor k.<br>
...
Si nos fijamos en los tumores clasificados correctamente, el error es pequeño.
...
Para cada uno de los coeficientes $\beta_i$, realizamos el siguiente contraste de significación
...
Ningún coeficiente del grupo BI-RADS, Density o Margin es significativo. <br>
...
Density no está muy correlacionada con ninguna otra variable. <br>
Eliminamos las variables BI-RADS y repetimos el contraste de significación de los coeficientes ($\alpha=0.05$).
...
Todos los coeficientes menos los de la variable Density son significativos.

Hagamos el contraste _Likelihood ratio tests_ para saber que modelo explica mejor las variables.

$$
\left\{ \begin{array}{ll}
H_0:& \mbox{El modelo sin variables BI-RADS es verdadero},\\
H_1:& \mbox{El modelo completo es verdadero}.
\end{array}\right.
$$
...
Rechazamos, por tanto, seleccionamos el modelo con las variables BIRAD.<br>
Además, comparemos los errores.

Càlculo del error en test para el modelo completo.
```{r}
...
Càlculo del error en test para el modelo sin las variables BR (BIRADS).
```{r}
...
**Nota:** 
No solo buscamos un modelo que reduzca el error de test, sinó uno que reduzca el número de pacientes que se realizan una biopsia innecesária (falsos positivos).
Por otra parte, recordemos que tratamos datos de cáncer y, por tanto, no queremos tumores malignos no seleccionados (falsos negativos).

### Árboles de clasificación

Para los árboles de clasificación no es necesario usar las variables dummies.

Antes de seleccionar los mejores parámetros, probemos el método.
```{r, fig.align='center'}
require(ISLR)
require(tree)
...
Ahora, validación cruzada para elegir la profundidad.
```{r, fig.align='center'}
set.seed(555)
...
Primero, veamos que obtenemos creando 1000 muestras de Bootstrap y usando los parámetros por defecto: mtry$=\sqrt(p)$, donde $p$ es el númro de predictores.
```{r}
set.seed(1515)
...
Antes de buscar los mejores parámetros, hacemos una prueba del método.
```{r, fig.align='center'}
...
boost_pred = predict(object = boost, newdata = data_gbm[test,], n.trees = 50, type = "response")
boost_pred   <- ifelse(boost_pred  >= 0.5,1,0)
...
```{r}
matrix(c(list_knn31,list_knn5, list_logReg_0, list_logReg, list_tree, list_rForest,
         list_rForest1, list_boost, list_kmeans), nrow=9, ncol=4, byrow=TRUE, 
       dimnames=list(c("knn (K=31)","knn (K=5)","Logistic Reg. (All)", "Log Reg. (Not BR)", 
                       "Tree (d=3)","Random Forest (mtry 2)", "Random Forest (mtry 1)",
...
Los mejores modelos són: knn (K=5) y Boosting (nt=50, d=1, shr=0.1, n.mino=10).
...
dimnames=list(c("Boosting LogAge (nt=50, d=1, shr=0.1, n.mino=10)"),c("Error", "Accuracy", "Accuracy_benign", "Accuracy_malign")) )
```
