...
Error en test.
```{r}
logReg_fit_0 <- predict(logReg,newdata=data_dum[test,-17], type='response')
...
dimnames=list(c("Tree (d=3)"),c("Error", "Accuracy", "Accuracy_bening", "Accuracy_malign")) )
...
accuracy_bening_randForest=(ct_randForest[1,1])/(ct_randForest[1,1] + ct_randForest[2,1])
accuracy_malign_randForest=(ct_randForest[2,2])/(ct_randForest[1,2] + ct_randForest[2,2])

list_rForest=c(error_randForest, accuracy_randForest, accuracy_bening_randForest,accuracy_malign_randForest)
matrix(list_rForest, nrow=1, ncol=4,
dimnames=list(c("Random Forest (mtry 2)"),c("Error", "Accuracy", "Accuracy_bening", "Accuracy_malign")) )
```
...
accuracy_bening_randForest1=(ct_randForest1[1,1])/(ct_randForest1[1,1] + ct_randForest1[2,1])
accuracy_malign_randForest1=(ct_randForest1[2,2])/(ct_randForest1[1,2] + ct_randForest1[2,2])

list_rForest1=c(error_randForest1, accuracy_randForest1, accuracy_bening_randForest1,accuracy_malign_randForest1)
matrix(list_rForest1, nrow=1, ncol=4,
dimnames=list(c("Random Forest (mtry 1)"),c("Error", "Accuracy", "Accuracy_bening", "Accuracy_malign")) )
```
...
accuracy_bening_boost=(ct_boost[1,1])/(ct_boost[1,1] + ct_boost[2,1])
accuracy_malign_boost=(ct_boost[2,2])/(ct_boost[1,2] + ct_boost[2,2])

list_boost=c(error_boost, accuracy_boost, accuracy_bening_boost,accuracy_malign_boost)
matrix(list_boost, nrow=1, ncol=4,
dimnames=list(c("Boosting (shrinkage 0.01)"),c("Error", "Accuracy", "Accuracy_bening", "Accuracy_malign")) )
```
...
set.seed(3573)
boost = gbm(Severity~. , distribution = "bernoulli", data = data_gbm[train,],
               n.trees = 150, shrinkage = 0.1, interaction.depth = 1, n.minobsinnode = 10)
...
list_boost=c(error_boost, accuracy_boost, accuracy_bening_boost,accuracy_malign_boost)
matrix(list_boost, nrow=1, ncol=4,
dimnames=list(c("Boosting (nt=150, d=1, shr=0.1, n.mino=10)"),c("Error", "Accuracy", "Accuracy_bening", "Accuracy_malign")) )
```

###Clustering para classificación.

Aunque el clustering es para aprendizaje no supervisado, en este apartado vamos a aplicar el algoritmo kmeans, con k=2, con la idea de ver si los datos se dividen de forma _natural_ según Bening/malign.
...
error_kmeans=(ct_kmeans[1,2]+ct_kmeans[2,1])/length(test)
accuracy_kmeans=1-error_kmeans

accuracy_bening_kmeans=(ct_kmeans[1,1])/(ct_kmeans[1,1] + ct_kmeans[2,1])
accuracy_malign_kmeans=(ct_kmeans[2,2])/(ct_kmeans[1,2] + ct_kmeans[2,2])

list_kmeans=c(error_kmeans, accuracy_kmeans, accuracy_bening_kmeans,accuracy_malign_kmeans)
matrix(list_kmeans, nrow=1, ncol=4,
dimnames=list(c("kmeans (scaled)"),c("Error", "Accuracy", "Accuracy_bening", "Accuracy_malign")) )
```
...
matrix(c(list_knn5, list_logReg_0, list_logReg, list_tree, list_rForest,
       ...
       dimnames=list(c("knn (K=5)","Logistic Reg. (All)", "Log Reg. (Not BR)", "Tree (d=3)",
                       ...
                       "Boosting (nt=150, d=1, shr=0.1, n.mino=10)", "kmeans (scaled)"),
                     c("Error", "Accuracy", "Accuracy_bening", "Accuracy_malign")) )
```
...
Los mejores modelos són: knn (K=5) y Boosting (nt=150, d=1, shr=0.1, n.mino=10).
...
Selecciona K=53; nos quedamo con k=37 (el segundo mejor), ya que k=53 són demasiados vecinos.

```{r}
knn_fit_logA=knn(data_logA[train,], data_logA[test,], cl=data_logA[train,]$Severity, k = 37)
...
accuracy_bening_knn_logA=(ct_knn_logA$t[1])/(ct_knn_logA$t[1] + ct_knn_logA$t[3])
accuracy_malign_knn_logA=(ct_knn_logA$t[4])/(ct_knn_logA$t[2] + ct_knn_logA$t[4])

list_knn_logA=c(error_knn_logA, accuracy_knn_logA, accuracy_bening_knn_logA,accuracy_malign_knn_logA)
matrix(list_knn_logA, nrow=1, ncol=4,
dimnames=list(c("Knn (K=37)"),c("Error", "Accuracy", "Accuracy_bening", "Accuracy_malign")) )
```
...
accuracy_bening_boost_logA=(ct_boost_logA[1,1])/(ct_boost_logA[1,1] + ct_boost_logA[2,1])
...
list_boost_logA=c(error_boost_logA, accuracy_boost_logA,
                  accuracy_bening_boost_logA,accuracy_malign_boost_logA)
matrix(list_boost_logA, nrow=1, ncol=4,
dimnames=list(c("Boosting LogAge (nt=50, d=2, shr=0.1, n.mino=10)"),c("Error", "Accuracy", "Accuracy_bening", "Accuracy_malign")) )
```

En este caso, también se observan mejoras.
