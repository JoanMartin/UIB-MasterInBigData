...
El método de las variables instrumentales se utiliza cuando puede existir correlación entre la variable dependiente e independiente de tu modelo. Para evitar que esto suceda,  se busca una/s variable/s instrumental/es $Z$ de manera que $Corr(Y,Z)=0$ pero que $Corr(X,Z)\neq 0$. 

El primer paso es buscar una regresión de la variable independiente $X$ con las variables instrumentales $Z_i$. Para saber con qué variables instrumentales es mejor realizar la regresión sobre $X$ utilizaremos el método de validación cruzada. 
...
$$X= \delta + \alpha Z_1 +\beta Z_2 +\gamma Z_3 + e $$

Vamos a seleccionar con validación cruzada cuál es la mejor combinación de variables instrumentales, es decir, qué coeficientes $\alpha, \beta, \gamma$ dejamos o anulamos. Notemos que tenemos $2^3=8$ combinaciones para anular o mantener los coeficentes anteriores y en consecuencia tenemos 8 modelos posibles.

Para cada modelo, se realizará 5 particiones de los datos, en los que se estimará los mejores coeficientes con 4 de los 5 trozos de los datos y con el quinto trozo se calculará el error de predicción. Manteniendo la misma partición, se volverá a calcular los mejores coeficientes del modelo, pero esta vez cambiando el trozo de validación. En consecuencia tendremos 5 errores de predicción para cada modelo, uno para cada trozo de la partición que hemos dejado para calcular el error de predicción. Se suman estos 5 errores y así obtenemos el error de predicción del modelo con el que estabamos trabajando.
...
Finalmente, podemos observar las cargas de las dos componentes en la tabla *Eigenvectors (CORR)*. Si nos fijamos en dicha tabla, todas la variables salvo *SMIN* tienen un mayor coeficiente en la primera componente principal. Esto nos indica que la primera componente principal es la que explica la mayor variabilidad para estas variables. En cambio la variable *SMIN* tiene una mayor coeficiente para la segunda componente principal, en consecuencia esta componente explica mayoritariamente la variabilidad en *SMIN*



##Ejercicio 15

**Utiliza los datos, en el chero "satisfacción" y haz un análisis de componentes princi-
pales.**

```{r, include=FALSE}
#carregam dades
load("C:/Users/maria/Documents/UIB/MADM/1rSemestre/AprenentatgeEstadisticPresaDecisions/satisfaccion.RData")
```
...
* _PLAYA _ : nivel de satisfacción respecto a la playa.
...
PCbiplot <- function(PC, x="PC1", y="PC2", colors=c('black', 'black', 'red', 'red')) {
        ...
        plot <- ggplot(data, aes_string(x=x, y=y)) + geom_point(alpha = 0.25, col="turquoise2")
        ...
        plot <- plot + coord_equal() + geom_text(data=datapc, aes(x=v1, y=v2, label=varnames), size = 4, vjust=1, hjust=1.25, color=colors[3])
